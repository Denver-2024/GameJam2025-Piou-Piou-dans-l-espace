...
func _emit_signal(id : int):
	if id == 1:
		for i in range(get_parent().get_child_count()):
			if get_parent().get_child(i) is CharacterBody3D and get_parent().get_child(i).name == "1":
				get_parent().get_child(i).get_node("camera/Camera3D/HUD").windows(true, "party_parameters")
		...
		printer.print_line("Seul l'hôte peut démarrer la partie !")
